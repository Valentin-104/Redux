...
draft: false
summary: Gemini를 API를 활용하여 멀티턴 대화 만들기 (채팅)
---

[Gemini](https://ai.google.dev) API를 사용하면 Google의 최신 생성형 AI 모델에 액세스할 수 있다. 이를 통해 ChatGPT 같은 챗봇을 만들고자 하였다. 링크로 들어가서 튜토리얼을 살펴보고 진행했다.
...
3. API 문서에 있는 [샘플 앱](https://github.com/google/generative-ai-js/tree/main/samples/web)을 참고하여, 응답을 받아서 채팅 기록 상태를 업데이트하는 함수를 만든다.
    - 기본적으로 모델은 전체 생성 프로세스를 완료한 후 응답을 반환한다. 전체 결과를 기다리지 않고 스트리밍을 사용하여 부분 결과를 처리하면 상호작용을 더 빠르게 달성할 수 있다.
